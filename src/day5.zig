...
pub fn challenge2() !void {
    ...
    consolidatedRanges = .empty;
    defer consolidatedRanges.deinit(allocator);
    ...
    std.mem.sort(Range, ranges.items, {}, compare);
    for (ranges.items) |range| {
        if (consolidatedRanges.items.len < 1) {
            try consolidatedRanges.append(allocator, range);
            continue;
        }

        if (range.min > consolidatedRanges.getLast().max) {
            try consolidatedRanges.append(allocator, range);
        } else {
            consolidatedRanges.items[consolidatedRanges.items.len - 1].max = @max(range.max, consolidatedRanges.getLast().max);
        }
    }

    for (consolidatedRanges.items) |range| {
...
fn compare(context: void, left: Range, right: Range) bool {
    _ = context;
